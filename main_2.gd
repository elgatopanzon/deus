...
class PipelineManager:
	...
	func _create_context_from_node(node: Node, components: Array, component_registry: ComponentRegistry) -> Dictionary:
		var context := {}
		for comp in components:
			var instance = component_registry.get_component(node, comp)
			if instance != null:
				context[comp.get_global_name()] = instance
		return context

	func _call_stage_or_pipeline(stage_or_pipeline, node: Node, context: Dictionary, component_registry: ComponentRegistry) -> void:
		if typeof(stage_or_pipeline) == TYPE_CALLABLE:
			...
			if not component_registry.components_match(node, requires, exclude):
				return
			self.run(stage_or_pipeline, node, component_registry, context)
			for key in _create_context_from_node(node, requires + optional, component_registry).keys():
				context[key] = _create_context_from_node(node, requires + optional, component_registry)[key]

	func run(pipeline_class: Script, node: Node, component_registry: ComponentRegistry, context_override = null) -> void:
		...
		if context_override == null:
			context = _create_context_from_node(node, requires + optional, component_registry)
		for stage in stages.keys():
			for fn_or_pipe in stages[stage]:
				_call_stage_or_pipeline(fn_or_pipe, node, context, component_registry)
...
class World extends Node:
	...
	func execute_pipeline(pipeline_class: Script, node: Node, context_override = null) -> void:
		pipeline_manager.run(pipeline_class, node, component_registry, context_override)
